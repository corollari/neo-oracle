...
async function getBNBGASPrice() {
  ...
  const binance = (await getJSON('https://api.binance.com/api/v3/ticker/24hr?symbol=GASBTC')).lastPrice / (await getJSON('https://api.binance.com/api/v3/ticker/24hr?symbol=BNBBTC')).lastPrice; // The USDT pairs have higher volume than the BTC ones but given that everythig else is based on USDT I decided to go with BTC here
  ...
  return [coingecko, messari, binance, hitbtc, gateio];
}

async function getPrice(ticker) {
  const BNBGASPrices = await getBNBGASPrice();
  const medianPriceBNBGAS = BNBGASPrices.sort()[2];

  const tickerPrice = Number((await getJSON(`https://dex-european.binance.org/api/v1/ticker/24hr?symbol=${ticker}`))[0].weightedAvgPrice); // Use a 24hr avergae instead of spot because BNC doesn't have much volume and this should make price manipulation harder
...
module.exports = { updateOracle, getPrice, getBNBGASPrice };
